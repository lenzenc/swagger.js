...
(function() {
  ...
  SwaggerResource = (function() {
    ...
    SwaggerResource.prototype.addModels = function(models) {
      var extendedModel, extendedModelName, model, modelName, swaggerModel, _i, _len, _ref, _results;
      if (models != null) {
        for (modelName in models) {
          if (this.models[modelName] == null) {
            if (models[modelName]["extends"] != null) {
              extendedModel = models[models[modelName]["extends"]];
            }
            if (extendedModel != null) {
              extendedModel.name = models[modelName]["extends"];
            }
            if (extendedModel != null) {
              extendedModelName = models[modelName]["extends"];
            }
  ...
  SwaggerModel = (function() {
    ...
    function SwaggerModel(modelName, obj, extendedObj) {
      var propertyName;
      this.name = obj.id != null ? obj.id : modelName;
      if (extendedObj != null) {
        this.extendedName = extendedObj.name;
      }
      this.properties = [];
      if (extendedObj != null) {
        for (propertyName in extendedObj.properties) {
          this.properties.push(new SwaggerModelProperty(propertyName, extendedObj.properties[propertyName]));
        }
      }
      for (propertyName in obj.properties) {
        this.properties.push(new SwaggerModelProperty(propertyName, obj.properties[propertyName]));
      }
      console.log(this.getMockSignature());
    }
    ...
    SwaggerModel.prototype.createXMLSample = function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        prop = _ref[_i];
        if ((prop.refModel != null) && prop.isCollection) {
          s.push("<" + prop.refModel.name + "s>");
          s.push(prop.refModel.createXMLSample());
          s.push("</" + prop.refModel.name + "s>");
        } else if ((prop.refModel == null) && prop.isCollection) {
          s.push("<" + prop.name + ">");
          ...
          if (prop.valuesString != null) {
            s.push(" (" + prop.valuesString + ")");
          }
          s.push("</" + prop.name + ">");
        }
      }
  ...
  SwaggerOperation = (function() {
    ...
    function SwaggerOperation(nickname, path, httpMethod, parameters, summary, notes, responseClass, errorResponses, resource, consumes, produces) {
      ...
      if (this.responseClass != null) {
        ...
        this.responseSampleXML = this.getSampleXML(this.responseClass, this.resource.models);
      }
      ...
      for (_i = 0, _len = _ref1.length; _i < _len; _i++) {
        ...
        parameter.sampleXML = this.getSampleXML(parameter.dataType, this.resource.models);
    ...
    SwaggerOperation.prototype.getSampleXML = function(dataType, models) {
      var isPrimitive, listType, modelName, val;
      listType = this.isListType(dataType);
      isPrimitive = ((listType != null) && models[listType]) || (models[dataType] != null) ? false : true;
      modelName = isPrimitive ? void 0 : (listType != null ? models[listType].name : models[dataType].name);
      val = isPrimitive ? void 0 : (listType != null ? models[listType].createXMLSample() : models[dataType].createXMLSample());
      if (val) {
        if (listType != null) {
          val = "<" + modelName + "s>" + val + "</" + modelName + "s>";
        }
        return this.formatXML(val);
      }
    };

    SwaggerOperation.prototype.formatXML = function(xml) {
      var formatted, i, indent, n, node, pad, padding, reg, _i, _j, _len, _ref;
      formatted = "";
      reg = /(>)(<)(\/*)/g;
      ...
      for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
        ...
        padding = "";
        for (n = _j = 0; 0 <= pad ? _j <= pad : _j >= pad; n = 0 <= pad ? ++_j : --_j) {
          if (n < pad) {
            padding += '  ';
          }
        }
